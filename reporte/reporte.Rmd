...
Metadocencia nace de una necesidad, pero sobre todo de la capacidad y ganas de ayudar. La comunidad ya cuenta con alrededor de `r signif(interes,digits = 2)` profesionales que se sumaron a la propuesta desde distintas partes del mundo.

A `r lubridate::month(lubridate::today()) - 3`  meses del comienzo, la comunidad cuenta con `r asistieron` docentes que ya tomaron este curso y que, sumado a más de `r (interes - asistieron)` que ya se preinscribieron, hacen más de `r interes` profesionales que se sumaron a la iniciativa.
...
ggplot() +
  ...
  #scale_color_gradient(low = "blue", high = "red", limits=c(6, 9), breaks=seq(6, 9, by=1)) +
  #guides(color= guide_legend(), size=guide_legend()) +
  #scale_size_continuous(limits=c(6, 9), breaks=seq(6, 9, by=1)) +
  #theme(text = element_text(family = "Courier New"))
```


```{r datosargentina, message=FALSE, warning=FALSE, include=FALSE}
...
```{r mapaargentina, echo=FALSE, message=FALSE, warning=FALSE}
ggplot() +
  geom_sf(data = mapaargentina, alpha = 1, aes(fill = !is.na(cantidad)), color = "gray") +
  theme_map() +
